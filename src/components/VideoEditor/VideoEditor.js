...
function VideoEditor() {
  ...
  const renderUploader = () => {
    return (
      <div className={"wrapper"}>
        <input onChange={(e) => uploadFile(e.target.files)} type="file" className="hidden" id="up_file" />
        <FileDrop onDrop={(e) => uploadFile(e)} onTargetClick={() => document.getElementById("up_file").click()}>
  ...
  const renderEditor = () => {
    return (
      ...
      <Editor videoUrl={videoUrl} videoBlob={videoBlob} setVideoUrl={setVideoUrl} timings={timings} setTimings={setTimings} />
    );
  };
